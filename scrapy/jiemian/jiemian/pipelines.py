...
class JiemianPipeline(object):
    ...
    def process_item(self, item, spider):
        try:
            ...
            desc_text = title
            print(title)
            file_log.writelines('开始爬取界面-娱乐-大牌--'+datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")+'\n')
            ...
            print('*******************************************************')
            sql="insert into news_3(title,url,desc_text,catalog,content,sp_name) values('"+title+"','"+url+"','"+desc_text+"','"+catalog+"','"+content+"','"+sp_name+"')"
            print(sql)
            self.conn.query(sql)
            self.conn.commit()
            return item
